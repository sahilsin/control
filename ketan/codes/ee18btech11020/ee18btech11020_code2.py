...
def Circle_N(n):
  ...
  x = np.around(x0 + r*np.cos(theta),decimals=4)
  y = np.around(y0 + r*np.sin(theta),decimals=4)  #rounding off
  x = np.expand_dims(x,axis=1)
  y = np.expand_dims(y,axis=1)
  Z = np.append(x,y,axis=1)
  return Z
...
Real,Imag,Freq = np.around(control.nyquist(G,w),decimals=4) 			# Rounding Off
NyQ_Data = np.append(np.expand_dims(Real,axis=1), np.expand_dims(Imag,axis=1), axis=1)
...
print ('The values of N,f are',Extract1(np.round(N,decimals=4),NyQ_Data,Freq))	#prints(N,frequency)
